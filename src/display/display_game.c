...
void		compute_walls_and_display_texture(t_display *display, double lin_len, int count, t_p *p_collision);
...
void	display_game(t_display *display)
{
	...
	while ( i <= 1)
	{
		...
		compute_walls_and_display_texture(display, lin_len, count, &p_collision);
		i += one_on_width;
		count++;
	}
}
